 ...
 */
export const productsGenerator = (quantity: number = 5, callback?: any): Array<any> => {
...
export const withOnSale = (rows: any[]) =>
  rows.map((row: { id: number; onSale: boolean }) => {
...
export const productsExpandRowsGenerator = (quantity = 5, callback: (v: unknown, k: number) => unknown) => {
  if (callback) return Array.from({ length: quantity }, callback);
